<template>
    <div class="center-container">
        ...
        <p class="text-center text-gray-600 mt-2">
          Enter the candidates informations
        </p>
        ...
        <form @submit.prevent="submitForm" class="mt-4">
          ...
          <div class="mb-4">
            <input
              type="tel"
              v-model="position"
              placeholder="Enter position contested for"
              class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400"
              required
            />
          </div>
    ...
    methods: {
      async submitForm() {
        ...
        try {
          ...
          this.successMessage = response.data.message || "Candidate registered successfully!";
          this.name = "";
          this.position = "";
          this.$router.push("/candidates");
        } catch (error) {
          this.errorMessage = error.response?.data?.message || "Failed to register candidate";
        } finally {
          this.loading = false;
        }
      },
